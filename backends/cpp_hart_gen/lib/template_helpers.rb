...
module CppHartGen
  module TemplateHelpers
    ...
    # Computed from enum_order and register counts defined in the register file YAMLs.
    # Must stay consistent with the enum layout generated by backends/cpp_hart_gen/templates/reg.hxx.erb.
    def rf_enum_base(rf_name)
      rf = cfg_arch.register_file(rf_name)
      raise "Unknown register file '#{rf_name}'" unless rf

      order = rf.enum_order
      raise "No enum_order defined in register file '#{rf_name}'" unless order

      cfg_arch.register_files
              .select { |other| other.enum_order && other.enum_order < order }
              .sum { |other| other.registers.length }
    end
